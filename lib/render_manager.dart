    ...
    tileResultPort.listen((message) async {
      ...
      final frame =
          await ImmutableBuffer.fromUint8List(
            ...
            (value) => ImageDescriptor.raw(
              value,
              width: response.width,
              height: response.height,
              pixelFormat: PixelFormat.bgra8888,
            ).instantiateCodec().then((codec) => codec.getNextFrame()),
          );
      ...
      if (waitingTiles.isNotEmpty) {
        isolateList[response.isolateId].toIsolate.send(
          waitingTiles.removeLast(),
        );
      } else {
